...
def page_demo(pipe: dict) -> None:
    ...
    for i, d in enumerate(detections):
        ...
        with st.expander(f"Placa #{i + 1} — {label}  (confianza media: {d['conf']:.0%})", expanded=True):
...
def page_visualizaciones(pipe: dict) -> None:
    ...
    if all_pairs:
        ...
        st.markdown(
            ...
            f"Alto promedio: **{avg_h:.1f}px** | Relación de aspecto media: **{avg_asp:.2f}**"
        )
    else:
    ...
    for idx, (fname, xmin, ymin, xmax, ymax, _) in enumerate(test_rows[:n_show]):
        ...
        col = cols[idx % 3]
        with col:
            st.image(bgr_to_rgb(enhanced), caption=f'OCR: "{text}" | chars={len(chars)}',
...
def page_metricas() -> None:
    ...
    # ── Matriz de confusión ────────────────────────────────────────────────────
    st.subheader("Matriz de confusión (36 clases A-Z + 0-9)")
    ...
    st.caption(
        ...
        "Valores fuera de la diagonal = confusiones (ej. I↔1, O↔0)."
    )
